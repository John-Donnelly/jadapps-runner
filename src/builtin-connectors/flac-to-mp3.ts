/**
 * flac-to-mp3: lossy re-encode FLAC → MP3 via libmp3lame. Loses fidelity
 * but cuts file size by 5-10x at typical bitrates.
 */
...
export default async function flacToMp3(ctx: ToolContext): Promise<StepResult> {
  const start = Date.now();
  const ref = ctx.fileRefs[0];
  if (!ref) return errorResult("missing_input", "flac-to-mp3 requires one audio input");
  const cfg = ctx.inputs ?? {};
  const bitrate = Math.max(32, Math.min(320, Math.floor(Number(cfg.bitrate ?? 256))));
