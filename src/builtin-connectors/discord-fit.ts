/**
 * discord-fit: re-encodes audio so the result fits Discord's free-tier
 * upload limit (8 MB at the time of writing). Picks an MP3 bitrate that
 * keeps total size under the cap given the source duration. If even
 * 32kbps mono can't fit, returns size_too_large.
 */
...
export default async function discordFit(ctx: ToolContext): Promise<StepResult> {
  const start = Date.now();
  const ref = ctx.fileRefs[0];
  if (!ref) return errorResult("missing_input", "discord-fit requires one audio input");
  const cfg = ctx.inputs ?? {};
  const targetMb = Math.max(1, Math.min(100, Number(cfg.targetMb ?? 8)));
  ...
  const targetBytes = targetMb * 1024 * 1024 * safetyMargin;
  ...
  const outRef = `${baseName}-discord.mp3`;
