/**
 * image-blur: applies a Gaussian blur to an input image.
 */
...
export default async function imageBlur(ctx: ToolContext): Promise<StepResult> {
  const start = Date.now();
  const ref = ctx.fileRefs[0];
  if (!ref) return errorResult("missing_input", "image-blur requires one image input");
  ...
  const sigma = Math.max(0.3, Math.min(1000, Number(cfg.sigma ?? 5)));
  const inPath = join(ctx.scratchDir, ref.ref);
  const totalIn = sizeOrFallback(inPath, ref.bytes);
  const buf = await readFile(inPath);
  ctx.emitProgress(totalIn);
  const out = await (sharp as unknown as (b: Buffer) => { blur(s: number): { toBuffer(): Promise<Buffer> } })(buf).blur(sigma).toBuffer();
  const outRef = (ref.filename ?? ref.ref).replace(/(\.[^.]+)$/, ".blurred$1");
