/**
 * linkedin-banner: 1584x396 personal-profile banner.
 */
...
export default async function linkedinBanner(ctx: ToolContext): Promise<StepResult> {
  const start = Date.now();
  const ref = ctx.fileRefs[0];
  if (!ref) return errorResult("missing_input", "linkedin-banner requires one image input");
  ...
  const out = await factory(buf).resize({ width: 1584, height: 396, fit: "cover", position: "attention" }).jpeg({ quality: 92 }).toBuffer();
  const outRef = "linkedin-banner.jpg";
  await writeFile(join(ctx.scratchDir, outRef), out);
  return { ok: true, outputs: { width: 1584, height: 396, outputBytes: out.length }, fileRefs: [{ ref: outRef, bytes: out.length, sha256: "", mime: "image/jpeg", filename: outRef }], bytesProcessed: totalIn, durationMs: Date.now() - start };
}
