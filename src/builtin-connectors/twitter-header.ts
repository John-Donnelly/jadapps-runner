/**
 * twitter-header: 1500x500 X (Twitter) profile header banner.
 */
...
export default async function twitterHeader(ctx: ToolContext): Promise<StepResult> {
  const start = Date.now();
  const ref = ctx.fileRefs[0];
  if (!ref) return errorResult("missing_input", "twitter-header requires one image input");
  ...
  const out = await factory(buf).resize({ width: 1500, height: 500, fit: "cover", position: "attention" }).jpeg({ quality: 92 }).toBuffer();
  const outRef = "twitter-header.jpg";
  await writeFile(join(ctx.scratchDir, outRef), out);
  return { ok: true, outputs: { width: 1500, height: 500, outputBytes: out.length }, fileRefs: [{ ref: outRef, bytes: out.length, sha256: "", mime: "image/jpeg", filename: outRef }], bytesProcessed: totalIn, durationMs: Date.now() - start };
}
