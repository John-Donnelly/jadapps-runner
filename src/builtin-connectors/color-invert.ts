/**
 * color-invert: inverts the colours of an input image (negate).
 */
...
export default async function colorInvert(ctx: ToolContext): Promise<StepResult> {
  const start = Date.now();
  const ref = ctx.fileRefs[0];
  if (!ref) return errorResult("missing_input", "color-invert requires one image input");
  ...
  const out = await (sharp as unknown as (b: Buffer) => { negate(o: { alpha: boolean }): { toBuffer(): Promise<Buffer> } })(buf).negate({ alpha: false }).toBuffer();
  const outRef = (ref.filename ?? ref.ref).replace(/(\.[^.]+)$/, ".inverted$1");
