/**
 * pinterest-long-pin: 1000x2100 long-pin format.
 */
...
export default async function pinterestLongPin(ctx: ToolContext): Promise<StepResult> {
  const start = Date.now();
  const ref = ctx.fileRefs[0];
  if (!ref) return errorResult("missing_input", "pinterest-long-pin requires one image input");
  ...
  const out = await factory(buf).resize({ width: 1000, height: 2100, fit: "cover", position: "attention" }).jpeg({ quality: 90 }).toBuffer();
  const outRef = "pinterest-long-pin.jpg";
  await writeFile(join(ctx.scratchDir, outRef), out);
  return { ok: true, outputs: { width: 1000, height: 2100, outputBytes: out.length }, fileRefs: [{ ref: outRef, bytes: out.length, sha256: "", mime: "image/jpeg", filename: outRef }], bytesProcessed: totalIn, durationMs: Date.now() - start };
}
