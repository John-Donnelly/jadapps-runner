/**
 * pdf-unlock: alias for pdf-remove-password — unlocks a password-protected
 * PDF using qpdf. Distinguished here as a separate tool because the
 * orchestrator catalogues it as a different intent (user has the password
 * vs. didn't realise it was protected).
 */
...
export default async function pdfUnlock(ctx: ToolContext): Promise<StepResult> {
  const start = Date.now();
  const ref = ctx.fileRefs[0];
  if (!ref) return errorResult("missing_input", "pdf-unlock requires one PDF input");
  ...
  return {
    ok: true,
    outputs: { unlockedBytes: outBytes },
