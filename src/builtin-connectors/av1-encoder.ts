...
export default async function av1Encoder(ctx: ToolContext): Promise<StepResult> {
  ...
  // Consult the host probe; pick av1_nvenc / av1_qsv / av1_amf when present.
  // For software, prefer libsvtav1 and fall back to libaom-av1 if the build
  // doesn't have it. preferSoftware skips hardware entirely.
  const preferSoftware = cfg.preferSoftware === true;
  const choice = selectVideoEncoder(await probeHardware(), "av1", { preferSoftware });

  let usedCodec = choice.encoder;
  if (choice.hardware) {
    try {
      await execFileAsync(
        "ffmpeg",
      ...
      if ((err as { code?: string }).code === "ENOENT") {
        return errorResult("driver_missing", "ffmpeg not found on PATH");
      }
      return errorResult("ffmpeg_error", `AV1 hardware encode failed: ${(err as { message: string }).message}`);
    }
  } else {
    // libsvtav1 first; fall back to libaom-av1 if this build doesn't have it.
    try {
  ...
  return {
    ...
    outputs: {
      ...
      crf: choice.hardware ? null : crf,
      speedPreset: choice.hardware ? null : speedPreset,
