/**
 * youtube-thumbnail: 1280x720 video thumbnail with high JPEG quality.
 */
...
export default async function youtubeThumbnail(ctx: ToolContext): Promise<StepResult> {
  const start = Date.now();
  const ref = ctx.fileRefs[0];
  if (!ref) return errorResult("missing_input", "youtube-thumbnail requires one image input");
  ...
  const out = await factory(buf).resize({ width: 1280, height: 720, fit: "cover", position: "attention" }).jpeg({ quality: 92 }).toBuffer();
  const outRef = "youtube-thumbnail.jpg";
  await writeFile(join(ctx.scratchDir, outRef), out);
  return { ok: true, outputs: { width: 1280, height: 720, outputBytes: out.length }, fileRefs: [{ ref: outRef, bytes: out.length, sha256: "", mime: "image/jpeg", filename: outRef }], bytesProcessed: totalIn, durationMs: Date.now() - start };
}
