/**
 * opus-to-mp3: Opus → MP3 transcode via ffmpeg.
 */
...
export default async function opusToMp3(ctx: ToolContext): Promise<StepResult> {
  const start = Date.now();
  const ref = ctx.fileRefs[0];
  if (!ref) return errorResult("missing_input", "opus-to-mp3 requires one audio input");
