/**
 * auto-white-balance: applies sharp's auto white-balance / normalise
 * pipeline to correct colour cast.
 */
...
export default async function autoWhiteBalance(ctx: ToolContext): Promise<StepResult> {
  const start = Date.now();
  const ref = ctx.fileRefs[0];
  if (!ref) return errorResult("missing_input", "auto-white-balance requires one image input");
  ...
  const out = await (sharp as unknown as (b: Buffer) => { normalise(): { toBuffer(): Promise<Buffer> } })(buf).normalise().toBuffer();
  const outRef = (ref.filename ?? ref.ref).replace(/(\.[^.]+)$/, ".awb$1");
