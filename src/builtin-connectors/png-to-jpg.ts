/**
 * png-to-jpg: re-encodes PNG as JPEG, configurable quality.
 */
...
export default async function pngToJpg(ctx: ToolContext): Promise<StepResult> {
  const start = Date.now();
  const ref = ctx.fileRefs[0];
  if (!ref) return errorResult("missing_input", "png-to-jpg requires one PNG input");
  ...
  const outRef = (ref.filename ?? ref.ref).replace(/\.png$/i, ".jpg");
