/**
 * m4a-to-mp3: AAC-in-M4A → MP3 transcode via ffmpeg. Useful when you
 * receive iPhone/voice-memo recordings and need broader compatibility.
 */
...
export default async function m4aToMp3(ctx: ToolContext): Promise<StepResult> {
  const start = Date.now();
  const ref = ctx.fileRefs[0];
  if (!ref) return errorResult("missing_input", "m4a-to-mp3 requires one audio input");
