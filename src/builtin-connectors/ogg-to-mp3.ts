/**
 * ogg-to-mp3: Vorbis/Ogg → MP3 transcode via ffmpeg.
 */
...
export default async function oggToMp3(ctx: ToolContext): Promise<StepResult> {
  const start = Date.now();
  const ref = ctx.fileRefs[0];
  if (!ref) return errorResult("missing_input", "ogg-to-mp3 requires one audio input");
